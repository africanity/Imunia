...
import { AlertCircle, Building2, MapPin, MapPinned, Plus, Trash2, X, Pencil, Loader2 } from "lucide-react";
...
export default function CentresPage() {
  ...
  const cards = useMemo(() => {
    ...
    return centers.map((center, index) => {
      const initials = center.name.substring(0, 2).toUpperCase();
      const regionName = center.district?.commune?.region?.name ?? null;
      const districtName = center.district?.name ?? null;
      ...
      return (
        ...
        >
          <div className="flex items-start gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-orange-400 to-orange-500 text-lg font-bold text-white shadow-sm ring-2 ring-white">
              {initials}
            </div>
